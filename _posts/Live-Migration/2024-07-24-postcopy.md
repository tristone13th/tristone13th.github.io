...
### `MIGRATION_STATUS_POSTCOPY_PAUSED` / QEMU
...
这个 bitmap 不是 dirty bitmap，每一个 bit 单纯表示的就是当前哪些 page 是没有接收到或者接收到后又 discard（不是通过 `RAMDiscardManager` 而是在 load 的过程中触发的 discard） 的（为 1 表示 destination 是有的）。
