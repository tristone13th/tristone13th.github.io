 ...
 * 魔法函数 \_\_getitem\_\_ 就是用来处理 something[key] 这种情况的。
 ...
 * Python 代码中，括号（大中小）中的换行都是被忽略呃，所以可以在括号的中间进行换行。尽量避免 \ 来换行，因为太丑。
 ...
 * 可以使用生成器表达式，这个东西的好处就是它是使用 yield 来生成元素的，所以它很节省空间。把列表推导式的中括号换成圆括号就可以了。
 ...
 * [] 当中的其实是一个 slice 对象（只有在括号中才可以被识别），我们也可以创建自己的 slice 对象，直接通过 slice() 来创建就好。
 ...
 * 如果一个类实现了 \_\_call\_\_ 内置函数，那么这个对象就是 callable 的。
 ...
 * Any number of arguments after the first are captured by *content as a tuple.
   > Any number of arguments after the first are captured by *content as a tuple. -> Page 175 (Underline).
 ...
 * 如果想让某参数只能被字典来指定，而不能通过位置来指定，把它放在具有 * 参数之后，这样子任何位置的参数都不会匹配为这个参数。
 ...
 * metaprogramming—changing pro‐ gram behavior at runtime
 ...
 * 注意，Python 没有函数重载！但是可以通过 singledispatch 来进行实现。
 ...
 * Python 当中的参数传递方式只有一个：call by sharing。也就是对要传入的地址进行复制。
 ...
 * 默认值存储在函数对象当中，如果其实可变的，那么会影响到函数的每一次调用。
 ...
 * del 删除名字，而不删除对象。对象被删除是垃圾回收机制的结果。
 ...
 * Python 不会关注 classmethod 的第一个名称是 cls 还是 self。
 ...
 * 可以直接在类中定义 \_\_slots\_\_，只要是一个 iterable 即可。这样子 Python 解释器会把它所有的属性值放在元组当中，从而防止了空间浪费。\_\_slots\_\_ 好像也可以优化时间，因为列表中的 get 和 set 操作的最差时间复杂度是 O(1)，因为在内存中更加紧凑。但是字典中是 O(n)。
 ...
 * sequence protocol 仅仅要求实现 \_\_len\_\_ 和 \_\_item\_\_ 即可。
 ...
 * In other words, indices exposes the tricky logic that's implemented in the built-in sequences to gracefully handle missing or negative indices and slices that are longer than the target sequence.
   > In  other  words,  indices  exposes  the  tricky  logic  that's  implemented  in  the  built-in sequences to gracefully handle missing or negative indices and slices that are longer than the target sequence. -> Page 308 (Underline).
 ...
 * 无论是添加属性还是修改属性，都会触发这个函数的执行。
   > \_\_setattr\_\_ -> Page 313 (Underline).

### Vector Take #4: Hashing and a Faster ==

 * When using reduce, it's good practice to provide the third argu‐ ment, reduce(function, iterable, initializer)
   > When using reduce, it's good practice to provide the third argu‐ ment,  reduce(function,  iterable,  initializer) -> Page 316 (Underline).
 ...
 * goose typing：使用 isinstance(obj, cls) 是可以接受的，只要 cls 是一个抽象基类（ABC），也就意味着它继承了 abc.ABCMeta。
 ...
 * 三个很基础的 ABC，第一个是 Iterable，只要实现了 \_\_iter\_\_ 就是 Iterable。第二个是 Container，只要实现了 \_\_contains\_\_ 就是 Container，第三个是 Sized，只要实现了 \_\_len\_\_ 就是 Sized。
 ...
 * Intergral 继承于 Rational，Rational 继承于 Real，Real 继承于 Complex，Complex 继承于 Number。
 ...
 * goose typing 的精髓是，我们可以注册一个类为一个 ABC 的虚拟子类，即使它并没有继承它。这是一种约定，解释器并不会进行检查，如果我们没有实现，那么就会出现一个运行时异常。
 ...
 * 一个 concrete class 最多继承一个 concrete class，其他的只能是 ABC 或者 Mixin。
 ...
 * 实现了 \_\_getitem\_\_ 就是 Iterable。
 ...
 * yield 字典中的本意有两个：生产（produce）和让位（give way）。在协程中这两个都是很贴切的，Python 协程可以通过 yield 返回值，同时也可以通过 yield 为其他程序的执行让位。
 ...
 * 这条比较重要：yield from 的参数可以是任何 iterable，因为会自动执行 iter() 来获取 iterator。
 ...
 * 当 Class 有一个属性（类属性），同时 instance 也有一个对象属性的话，对象的属性会覆盖掉 Class 的属性（非 property 的情况）——至少对于从 instance 来获取属性是如此。
 ...
 * 然而，通过 \_\_dict\_\_ 来更改 instance 的属性值，在属性获取时并不会更改，因为其并不能覆盖掉通过 property 装饰器所装饰的属性。
 ...
 * 直接读和写 \_\_dict\_\_ 并不会触发特殊函数，比如 \_\_setattr\_\_, \_\_getattr\_\_ 这些，这是经常用的将它们进行 bypass 掉的方式。
 ...
 * 在使用描述符的 \_\_set\_\_ 设置属性值时，为什么使用 instance 的 \_\_dict\_\_ 而不使用 self 的 \_\_dict\_\_ 来存储值呢？这是因为描述符实例是管理类的类属性，而不是对象属性，这意味着这些描述符实例在多个管理类实例之间是共享的，因此这些值也是多个实例共享的，这很明显是不合理的。
 ...
 * 如果我们想进一步省去写参数名字，因为可能出错，那么我们可以通过这种实现方式，也就是通过一个 counter，每次初始化一个实例时自增一，并把这个属性（因为有 counter 的存在，所以我们可以保证每次的属性名都是唯一的）存储在要管理的对象当中，这样子就可以避免名字发生冲突。
 ...
 * 描述符在类一级的表现仍然具有这种非对称性：如果是读取类属性，那么会被描述符的 \_\_get\_\_ 处理；如果是设置类属性，那么描述符的 \_\_set\_\_ 则不会处理。我认为这种设计方式符合直觉。
 ...
 * 用户定义的类中定义的函数都有一个 \_\_set\_\_ 方法，因此他们也是描述符。
 ...
 * 每一个类都隐性或者显性地是 type 的实例，但是仅仅 metaclass 能够使 type 的子类。
 ...
 * \_\_prepare\_\_ 只能够在元类当中使用，同时他必须是 @classmethod。\_\_prepare\_\_ 的作用是决定属性的顺序。
