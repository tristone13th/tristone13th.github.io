...
### 系统调用的 context switch 会改 CR3 吗？/ 内核页表 / 进程页表 / Meltdown
...
There is no need to duplicate the kernel page tables for each process, ie. all process-specific page directories can reference the same set of kernel page tables. 尽管 Virtual memory 空间 duplicate 的，但是其实用的物理内存并没有。页表也是要存在内存里的，所以指向内核页表的物理地址的页表项大家都保持一致就可以啦。
...
我觉得应该是有的，每一个进程都有自己的内核栈。内核栈不能被用户态访问所以明显映射其 PTE 的 U/S bit 应该是 S，同时每一个进程的内核栈内容又不尽相同，所以不能放在内核页表那部分里面。

除了内核栈，我觉得在内核执行期间分配的任何内存都应该不能被用户态所访问，所以映射其 PTE 的 U/S bit 也应该是 S，这也不应该是内核页表那部分里面的。

综上所述，除了内核页表和用户页表，还有一些其他的部分是 process-specific 同时又是只能是 kernel 来访问的，这应该就是其对应内核线程所分配的那一部分内存。

这一部分内存在 process 关闭之后会被自动回收吗？理论上来说，当关掉一个进程时，其页表里所映射的所有页都应该被回收。但是内核态分配的内存比如 kmalloc 就不会被回收，为什么要这么设计？如果没有被映射，其所对应的进程已经关掉了，那又在谁的页表里呢？

**猜想**：Kernel 里肯定在一个固定的内存位置有一个数据结构记录着每一个物理页有没有被某一个进程所映射。我觉得 Userspace 的回收和 kernel space 的回收说的不是同一回事。Userspace 回收表示的是从其页表里拿掉了，但是其实在 array 里把对应的物理页标成没有任何进程使用的 free 状态才算是回收了。但是所有 PTE 为 S 的页不能被标，因为至少内核页表是大家 shared 的，所以有其他人在用。因此其他非内核页表的 PTE 标为 S 的页应该也不能回收。可能因为不好区分？
...
你可能会说这样的话用户态可以更改其栈内容，对内核放进去的数据造成破坏，但是当从内核返回用户的时候，内核的数据已经都 pop 出来用完了，所以用户态也无从更改了。

Preserve the integrity of the kernel, otherwise the process (another thread, for example) could do some nasty things to the stack. 我能想到的方式是另一个进程和这个进程共享映射了其栈区域，如果这个进程运行在 kernel space 并且往栈上加了东西，那么这个另一个线程可以更改线程的内容从而修改内核放在栈上的数据。
